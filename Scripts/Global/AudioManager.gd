...
func create_3d_audio_at_location(location: Vector3, type: Utils.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_3D_audio.pitch_scale += Utils.get_rng(-sound_effect.pitch_randomness, sound_effect.pitch_randomness )
...
func create_audio(type: Utils.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		var sound_effect: SoundEffect = sound_effect_dict[type]
		if sound_effect.has_open_limit():
			sound_effect.change_audio_count(1)
			...
			new_audio.pitch_scale += Utils.get_rng(-sound_effect.pitch_randomness, sound_effect.pitch_randomness )
